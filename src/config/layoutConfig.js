            ...
            <div style="width:100%; display:flex; flex-direction:column; align-items:center; gap:8px; margin-bottom:10px;">
                <nav class="footer-nav" style="width:100%; display:flex; flex-direction:column; align-items:center; gap:8px;">
                    ${footerConfig.navigation.links.map(link =>
                        `<a href="${link.href}" class="footer-link" style="font-size:16px; color:#ccc; text-decoration:none; padding:8px 0; border-radius:6px; width:100%; text-align:center; background:rgba(255,255,255,0.03); transition:background 0.2s;">${wrapTextInGlowSpans(link.text)}</a>`
                    ).join('')}
                </nav>
            </div>
